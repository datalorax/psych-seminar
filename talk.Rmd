...
```{r state-tuition3-eval, fig.height = 8}
ggplot(tuition, aes(State, `2015-16`)) +
  geom_col() +
  coord_flip() +
...
```{r state-tuition4-eval, fig.height = 8}
...
```{r oregon-highlight-eval, echo = FALSE, fig.height = 8}
ggplot(tuition, aes(fct_reorder(State, `2015-16`), `2015-16`)) +
  geom_col() +
...
.pull-left[
```{r iris_lines2, fig.width = 7}
iris_lines
```
]

.pull-right[
```{r iris_filled2, fig.width = 7}
iris_filled
```
]
